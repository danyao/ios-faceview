...
class FaceView: UIView {
  ...
  private func pathForMouth() -> UIBezierPath {
    ...
    let rect = CGRect(x: skullCenter.x - mouthWidth / 2,
    ...
    let smileOffset = CGFloat(max(-1, min(1, mouthCurvature))) * rect.height
    let cp1 = CGPoint(x: start.x + mouthWidth / 3, y: start.y + smileOffset)
    let cp2 = CGPoint(x: end.x - rect.width / 3, y: start.y + smileOffset)
  ...
  override func draw(_ rect: CGRect) {
    lineColor.set()  // sets stroke and fill
    pathForSkull().stroke()
